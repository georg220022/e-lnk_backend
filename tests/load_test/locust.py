...
class TestMainFunc(HttpUser):
    """Нагрузочное тестирование основного функционала сайта, при запуске
    не забыть отключить тротлинг"""
    ...
    def open_settings(self):
        # Открываем настройки
        self.client.get(
            "api/v1/settings", headers={"Authorization": f"Bearer {self.token}"}
        )
    ...
    def open_panel(self):
        # Открываем панель
        self.client.get(
            "api/v1/panel", headers={"Authorization": f"Bearer {self.token}"}
        )
    ...
    def on_start(self):
        ...
        response = self.client.post(
            "api/v1/login",
            json={"email": "Почта_пользователя", "password": "пароль_пользователя"},
        )
