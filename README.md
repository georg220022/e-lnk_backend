...
- ОПТИМИЗАЦИЯ: Рассчеты производятся во время открытия панели, если у пользователя появляется более 5000 не обработанных данных, во избежание подвисаний при обращении к панели - данные рассчитываются в фоновом режиме с последующим удалением подсчитанных данных (Celery task).
...
- ЛОГГИРОВАНИЕ: выполяется стандартыми инструментами Django
...
- ТРОТЛИНГ:  выполяется стандартыми инструментами Django
...
### Технологии
##### Django 4, DRF, Redis, PostgresSQL, Docker, Python 3, Nginx, Gunicorn, Locust, Ubuntu-server, Celery 
